...
export default class BetterTTVEmoteProvider extends AbstractEmoteProvider implements IAbstractEmoteProvider {
	// TODO decouple from PROVIDER_ENUM and use a unique identifier
	id = PROVIDER_ENUM.BETTERTTV
	name = 'BetterTTV'
	...
	async fetchEmotes({ userId, channelId }: ChannelData) {
		info('EXT:BTTV', 'EMOT:PROV', 'Fetching emote data from BetterTTV..')
		this.status = EmoteProviderStatus.LOADING

		if (!userId) {
			this.status = EmoteProviderStatus.CONNECTION_FAILED
			throw new Error('Missing Kick user id for BetterTTV provider.')
		}

		const isChatEnabled = !!this.settingsManager.getSetting(channelId, 'ext.betterttv.show_emotes')
		if (!isChatEnabled) {
		...
		const [globalData, userData] = await Promise.all([
			REST.get(`https://api.betterttv.net/3/cached/emotes/global`).catch(err => {
				error('EXT:BTTV', 'EMOT:PROV', 'Failed to fetch BetterTTV global emotes:', err)
			}),
			REST.get(`https://api.betterttv.net/3/cached/users/${PLATFORM}/${userId}`).catch(err => {
				error('EXT:BTTV', 'EMOT:PROV', 'Failed to fetch BetterTTV user emotes:', err)
			})
		])

		if (!globalData) {
			this.status = EmoteProviderStatus.CONNECTION_FAILED
			return error('EXT:BTTV', 'EMOT:PROV', 'Failed to fetch BetterTTV global emotes.')
		}
		...
		if (!globalEmoteSet) {
			this.status = EmoteProviderStatus.CONNECTION_FAILED
			return error('EXT:BTTV', 'EMOT:PROV', 'Failed to unpack global emotes from BetterTTV provider.')
		}
		...
		if (userEmoteSet) {
			...
			log(
				'EXT:BTTV',
				'EMOT:PROV',
				`Fetched ${globalEmoteSet.length + userEmoteSet.length} emote ${plural} from BetterTTV.`
			)
		} else {
			log('EXT:BTTV', 'EMOT:PROV', `Fetched ${globalEmoteSet.length} global emote set from BetterTTV.`)
		}
